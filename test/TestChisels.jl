    ...
    for a in 1:valence
        for e in 1:valence
            if eng[a] && eng[e] 
                ...
                v = zeros( valence) 
                v[e] = 1.0
                @assert isapprox(Dleto.__dist(uc,(u - v)), 0.0)  "UniversalChisel nullity incorrect."
            end
        end
    end
    return true
...
function testTuckerChiselEngaged(eng::Vector{Bool})
    ...
    for a in 1:valence
        if !eng[a]
            u = zeros(valence)
            u[a] = 1.0
            @assert isapprox(Dleto.__dist(tc,u) , 0.0 )  "TuckerChisel image incorrect, $eng."
        end
    end
    return true
end;
...
function testCentroidChiselEngaged(eng::Vector{Bool})
    ...
    @assert isapprox(Dleto.__dist(cc,u), 0.0)  "CentroidChisel nullity incorrect\r\n $cc \r\n $u."
    return true
end;
...
function testAdjointChisel(valence::Integer)
    ...
    for a = 1:(valence-1)
        for e = (a+1):valence
            ...
            v = zeros(valence)
            v[e] = 1.0
            @assert isapprox(Dleto.__dist(ac,u+v), 0.0)  "AdjointChisel nullity incorrect $ac\r\n $u\r\n $v."
        end
    end
    return true
end;
