...
@testset "List Operators Symmetries Tests" begin
    for val = 1:10
        @testset "No Symmetry Valency $val Tests" begin
            for _ = 1:10
                ...
                @test testReducedbyEngaged(LOp,20)
                linlocalops = rand(1:length(LinOps), val) .|> (i-> LinOps[i] )
                linLOp = Dleto.SymListOperators(axisdim,linlocalops,[i for i=1:val])
    ...
    for sym in syms 
        @testset "Symmetry $sym Tests" begin
            for _ = 1:10
                ...
                @test testInverse(LOp, 100)
                @test testReducedbyEngaged(LOp,20)
                @test testSymmetry(LOp, 100, rlocalops, sym)

                linlocalops = rand(1:length(LinOps), val) .|> (i-> LinOps[i] )
                rlinlocalops = [ linlocalops[abs(sym[i])] for i=1:length(sym)]
