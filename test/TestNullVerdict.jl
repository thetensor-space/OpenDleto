...
@testset "GramSolver(gram_eltype = ...)" begin
    @testset "gram_eltype = nothing reproduces the unmodified solver" begin
        ...
        # `GramSolver` DOES take `seed` now (`wants_seed(::GramSolver) = true`,
        # NullSolvers.jl) -- pass it explicitly rather than reseeding the
        # global rng around each call, so the two calls below agree bit-for-
        # bit regardless of what the global rng's state happens to be.
        r1 = Dleto.solve(Dleto.GramSolver(), L; nv = 8, seed = 4243)
        r2 = Dleto.solve(Dleto.GramSolver(gram_eltype = nothing), L; nv = 8, seed = 4243)
    ...
    @testset "gram_eltype = Float32 finds the same subspace as Float64" begin
        ...
        M[:, 1:4] .= 0.0
        L = LinearMaps.LinearMap(M)
        r64 = Dleto.solve(Dleto.GramSolver(), L; nv = 8, seed = 4243)
        r32 = Dleto.solve(Dleto.GramSolver(gram_eltype = Float32), L; nv = 8, seed = 4243)
